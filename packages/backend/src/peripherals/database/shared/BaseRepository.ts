...
export abstract class BaseRepository {
  ...
  protected knex(trx?: Knex.Transaction): Promise<Knex | Knex.Transaction> {
    return this.database.getKnex(trx)
  }

  /* eslint-disable @typescript-eslint/no-explicit-any */
  autoWrap<T>(obj: T): void {
  ...
  protected wrapAddMany<T extends AddManyMethod>(method: T): T {
    const fn = async (records: T[]): ReturnType<AddManyMethod> => {
      if (records.length === 0) {
        return []
      }
  ...
  private wrap<T extends AnyMethod>(
    ...
    const fn = async (...args: Parameters<T>): Promise<ReturnType<T>> => {
      const result = (await method.call(this, ...args)) as Awaited<
        ReturnType<T>
      >
      log(result)
